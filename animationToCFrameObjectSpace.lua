...
conn = game:GetService("RunService").RenderStepped:Connect(function()
	...
	for i,v in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
		if v:IsA("BasePart") then
			anims[#anims][v.Name]= tostring(game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame:ToObjectSpace(v.CFrame))
		end
	end
end)
end
...
local function import()
	...
	for i1,v1 in pairs(parsed) do
		for i2,v2 in pairs(v1) do
			...
			parsed[i1][i2] = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame:ToWorldSpace(CFrame.new(table.unpack(split)))
		end
	end
